                    ...
                    withVault([configuration: configuration, vaultSecrets: secretsProd]) {   
                        ...
                        sh '''
                            ...
                            helm upgrade $appName \
                                ...
                                --set secret.aws.s3.access_key=$AWS_BUCKET_ID \
                                --set secret.aws.s3.secret_key=$AWS_SECRET_KEY \
                                --set secret.aws.s3.case_bucket_name=$CASE_BUCKET_NAME \
                                --set secret.aws.s3.history_bucket_name=$HISTORY_BUCKET_NAME \
                                --set secret.aws.s3.signed_url_method=$SIGNED_URL_METHOD \
                                --set secret.firebase.firebase_debug=$FIREBASE_DEBUG \
                                --set secret.firebase.firebase_project_id=$FIREBASE_PROJECT_ID \
                                --set secret.firebase.firebase_client_email=$FIREBASE_CLIENT_EMAIL \
                                --set secret.firebase.firebase_client_id=$FIREBASE_CLIENT_ID \
                                --set secret.firebase.firebase_private_key=$FIREBASE_PRIVATE_KEY \
                                --set secret.firebase.firebase_client_x509_cert_url=$FIREBASE_CLIENT_X509_CERT_URL \
                                --set secret.firebase.firebase_database_url=$FIREBASE_DATABASE_URL \
                                --set secret.firebase.firebase_private_key_id=$FIREBASE_PRIVATE_KEY_ID \
                                --set secret.pubsub.pubsub_project_id=$PUBSUB_PROJECT_ID \
                                --set secret.pubsub.pubsub_private_key_id=$PUBSUB_PRIVATE_KEY_ID \
                                --set secret.pubsub.pubsub_private_key=$PUBSUB_PRIVATE_KEY \
                                --set secret.pubsub.pubsub_client_email=$PUBSUB_CLIENT_EMAIL \
                                --set secret.pubsub.pubsub_client_id=$PUBSUB_CLIENT_ID \
                                --set secret.pubsub.pubsub_auth_uri=$PUBSUB_AUTH_URI \
