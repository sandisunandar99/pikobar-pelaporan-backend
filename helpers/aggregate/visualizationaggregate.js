...
const visualizationAggregate = async (query, user) => {
  ...
  const conditions = [{ $match: { $and: [search, { ...WHERE_GLOBAL }] } },
  { $lookup: {
      from: "histories", localField: "last_history",
      foreignField: "_id", as: "last_history"
    }
  }, { $unwind: "$last_history" },
  {
    "$facet": {
  ...
  {
    "$project": {
      "visualization": "$visualization", "date_weekly": "$date_weekly",
